        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     * 
     * 在{@link #ArrayList()} 或 {@link #ArrayList(collection)} 构造方法中，
     ...
     */
    transient Object[] elementData; // non-private to simplify nested class access 不使用私有修饰，简化嵌套类访问
    ...
    public ArrayList(int initialCapacity) {
        ...
        // 初始化容量等于0时，使用EMPTY_ELEMENTDATA 对象
        } else if (initialCapacity == 0) {
            this.elementData = EMPTY_ELEMENTDATA;
        // 初始化容量小于0时，抛出IllegalArgumentException 异常
        } else {
    ...
    public ArrayList(Collection<? extends E> c) {
        ...
        if ((size = elementData.length) != 0) {
            ...
            // (see e.g. https://bugs.openjdk.java.net/browse/JDK-6260652) 该bug在JDK9之前的版本都会存在问题
            // 如果集合元素不是Object[] 类型，则会创建新的Object[] 数组，并将elementData赋值到其中
            // 这个判断用于解决BUG JDK-6260652，具体可以查看有道云笔记
    ...
    boolean batchRemove(Collection<?> c, boolean complement,
        ...
        // <2> 设置开始写入 w 为 r ，注意不是 ++r 。是 r++
        // r++ 后，用于读取下一个位置的元素。因为通过上面的优化循环，我们已经确认 es[r] 是不符合条件的。
        int w = r++;
        try {
            // <3> 继续遍历 elementData 数组，如果符合条件，则进行移除
